...
export const ProtocolEditor = ({
  ...
  const hasChanges = plainText !== originalText;

  // Initialize plain text when dialog opens
  useEffect(() => {
    if (open && protocol) {
      const text = protocolToPlainText(protocol);
      setPlainText(text);
  ...
  const performSave = async () => {
    ...
    try {
      ...
      toast({ title: "Sparat", description: "Protokollet har sparats.", duration: 1500 });
    } catch (error) {
      console.error("Failed to save protocol draft:", error);
      toast({
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={(v) => { if (!v) attemptClose(); else onOpenChange(v); }}>
        <DialogContent
          className="max-w-4xl w-[95vw] h-[90vh] p-0 gap-0 flex flex-col overflow-hidden [&>button:last-child]:hidden"
      ...
      <ConfirmDialog
        ...
        description="Du har ändringar som inte sparats. Vill du stänga utan att spara?"
