...
export function EnterpriseSettingsIntegrations({ settings, locks, canEdit, onUpdate }: Props) {
  ...
  const isDirty = useMemo(() => {
    const integrationsChanged = INTEGRATIONS.some(({ key }) => integrationStates[key] !== ((settings as any)?.[key]?.enabled ?? false));
    return integrationsChanged || apiAccessEnabled !== (settings.apiAccessEnabled ?? false) || webhooksEnabled !== (settings.webhooksEnabled ?? false) || customIntegrationsEnabled !== (settings.customIntegrationsEnabled ?? false);
